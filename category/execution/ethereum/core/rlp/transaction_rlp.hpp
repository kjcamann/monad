...
concept NullptrOrRef = std::same_as<Out, std::nullptr_t> ||
                       std::same_as<Out, std::reference_wrapper<T>>;

// If raw_transactions is non-null, appends a view of transaction. Each view
// aliases enc's underlying buffer, so it is valid only while that buffer lives,
// and is independently decodable with decode_transaction.
