...
intrinsic_gas_breakdown(Transaction const &tx) noexcept
{
    if constexpr (traits::evm_rev() < EVMC_HOMESTEAD) {
        ...
        return monad_c_evm_intrinsic_gas{
            .base = 21'000,
            .data = g_data<traits>(tx),
            .creation = g_txn_create(tx) + g_extra_cost_init(tx),
            .access_list = g_access_and_storage(tx),
            .authorizations = 0};
    }
    else {
...
uint64_t sum(monad_c_evm_intrinsic_gas const &g) noexcept
{
    return g.base + g.data + g.creation + g.access_list + g.authorizations;
}
