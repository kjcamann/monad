...
TEST(MonadNext, bn_add)
{
    auto const tests = transform_test_cases(
        load_test_cases(test_resource::geth_vectors_dir / "bn256Add.json"),
        [](auto &test) { test.gas *= 2; });

    do_geth_tests<MonadTraits<MONAD_NEXT>>("bn_add", tests, 0x06_address);
}

TEST(MonadNext, bn_mul)
{
    auto const tests = transform_test_cases(
        load_test_cases(
            test_resource::geth_vectors_dir / "bn256ScalarMul.json"),
    ...
    do_geth_tests<MonadTraits<MONAD_NEXT>>("bn_mul", tests, 0x07_address);
}

TEST(MonadNext, bn_pairing)
{
    auto const tests = transform_test_cases(
        load_test_cases(test_resource::geth_vectors_dir / "bn256Pairing.json"),
        [](auto &test) { test.gas *= 5; });

    do_geth_tests<MonadTraits<MONAD_NEXT>>("bn_pairing", tests, 0x08_address);
}

TEST(MonadNext, blake2f_valid)
{
    auto const tests = transform_test_cases(
        load_test_cases(test_resource::geth_vectors_dir / "blake2F.json"),
        [](auto &test) { test.gas *= 2; });

    do_geth_tests<MonadTraits<MONAD_NEXT>>(
