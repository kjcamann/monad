...
constexpr char const IN_PROGRESS_DIR[] = ".inprogress";

static int format_block_tmp_path(
    uint64_t block_number, char *path_buf, size_t path_buf_size)
{
    int const rc = snprintf(
        path_buf,
        path_buf_size,
        "%s/%lu",
        IN_PROGRESS_DIR,
    ...
    int *const fd_out)
{
    ...
    dirfd = monad_bcap_block_archive_get_dirfd(bca);
    rc = monad_bcap_block_archive_format_block_path(
    ...
    *(char *)mempcpy(subdir_buf, path_buf, (size_t)(subdir_end - path_buf)) =
    ...
    if (open_flags & O_CREAT) {
        ...
        if (mkdirat(dirfd, IN_PROGRESS_DIR, dir_create_mode) == -1 &&
            errno != EEXIST) {
            return FORMAT_ERRC(
                errno,
                "unable to create in-progress directory %s to write block %lu",
                IN_PROGRESS_DIR,
                (unsigned long)block_number);
        }
        rc = format_block_tmp_path(
            block_number, tmp_path_buf, sizeof tmp_path_buf);
        if (rc != 0) {
            return rc;
        }
    }
    fd = openat(
        dirfd,
        open_flags & O_CREAT ? tmp_path_buf : path_buf,
        open_flags,
        file_create_mode);
    if (fd == -1) {
        return FORMAT_ERRC(
            errno,
            "could not open %s [block %lu]",
            O_CREAT ? tmp_path_buf : path_buf,
            (unsigned long)block_number);
    }
    if (fd_out != nullptr) {
        *fd_out = fd;
    ...
    struct monad_evcap_writer *evcap_writer, char const *path_buf)
{
    ...
    char tmp_path_buf[64];
    int rc;
    int const dirfd = monad_bcap_block_archive_get_dirfd(bca);
    if (path_buf == nullptr) {
        rc = monad_bcap_block_archive_format_block_path(
    ...
    rc = format_block_tmp_path(block_number, tmp_path_buf, sizeof tmp_path_buf);
    if (rc != 0) {
        return rc;
    }
#if defined(__linux__)
    rc = renameat2(dirfd, tmp_path_buf, dirfd, path_buf, RENAME_NOREPLACE);
#elif defined(__APPLE__)
    rc = renameatx_np(dirfd, tmp_path_buf, dirfd, path_buf, RENAME_EXCL);
#else
    #error add support for O_EXCL-style rename on this platform
#endif
    if (rc == -1) {
        rc = FORMAT_ERRC(
            errno,
            ...
            (unsigned long)block_number,
            path_buf);
        (void)unlinkat(dirfd, tmp_path_buf, 0);
    }
