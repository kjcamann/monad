    ...
    char const **subdir_end)
{
    ...
    rc = snprintf(path_buf, path_buf_size, "%lu", group_number);
    if (rc < 0) {
    ...
    int *const fd_out)
{
    ...
    if (open_flags & (O_CREAT | O_TMPFILE)) {
        char subdir_buf[32];
        *(char *)mempcpy(
            subdir_buf, path_buf, (size_t)(subdir_end - path_buf)) = '\0';
        if (mkdirat(bca->dirfd, path_buf, dir_create_mode) == -1 &&
            errno != EEXIST) {
            return FORMAT_ERRC(
                errno,
    ...
    fd = openat(bca->dirfd, path_buf, open_flags, file_create_mode);
    if (fd == -1) {
        return FORMAT_ERRC(
