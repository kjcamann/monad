    ...
    uint64_t max_block, struct monad_bcap_block_range_list *missing_ranges)
{
    ...
    for (uint64_t subdir = min_subdir; subdir <= max_subdir;
        ...
        sprintf(subdir_buf, "%lu", (unsigned long)subdir);
        if (fstatat(bca->dirfd, subdir_buf, &subdir_stat, 0) == -1) {
            struct monad_bcap_block_range *this_range;
            ...
            // ENOENT case: this entire directory is missing
            last_range =
                TAILQ_LAST(&missing_ranges->head, monad_bcap_block_range_head);
            if (last_range != nullptr && last_range->max + 1 == subdir) {
                last_range->max = max_missing_in_subdir;
            }
            else {
                this_range = malloc(sizeof *this_range);
                if (this_range == nullptr) {
                    rc = FORMAT_ERRC(errno, "malloc failed");
                    goto Error;
                }
                this_range->min = min_missing_in_subdir;
                this_range->max = max_missing_in_subdir;
                TAILQ_INSERT_TAIL(&missing_ranges->head, this_range, next);
            }
            continue;
        }
        ...
        for (size_t s = 0; s < subdir_scan.len; ++s) {
            ...
            if (last_block != 0 && this_block != last_block + 1) {
                ...
                missing = malloc(sizeof *missing);
                if (missing == nullptr) {
                    rc = FORMAT_ERRC(errno, "malloc failed");
                    goto Error;
                }
                missing->min = last_block + 1;
                missing->max = this_block - 1;
                TAILQ_INSERT_TAIL(&missing_ranges->head, missing, next);
            }
