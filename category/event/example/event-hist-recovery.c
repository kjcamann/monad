...
static struct block_range compute_missing_block_range(
    uint64_t last_finalized_block, struct monad_event_ring_iter *iter,
    int pidfd)
{
    ...
    if (!monad_exec_get_block_number(
            iter->event_ring, &event, payload, &r.block_end)) {
...
RecoverAgain:
    monad_event_ring_iter_reset(&ring_iter);
    ...
    do {
        ...
        for (uint64_t b = missing.block_start + 1; b <= missing.block_end;
            ...
            while (monad_evcap_event_iter_next(
