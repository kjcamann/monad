...
static void fetch_missing_block_range(
    char const *fetch_command, char const *block_dir_path, block_range_t range,
    bool verbose)
{
    char start_block[32];
    char end_block[32];
...
static block_range_t compute_missing_block_range(
...
RecoverAgain:
    block_range_t missing =
        compute_missing_block_range(last_finalized_block, event_ring, pidfd);
    do {
        ...
        for (uint64_t b = missing.block_start + 1; b <= missing.block_end;
            ...
            if (monad_bcap_block_archive_open_block_reader(
                    block_archive,
                    b,
                    nullptr,
                    0,
        ...
        missing =
            compute_missing_block_range(missing.block_end, event_ring, pidfd);
    }
    while (g_should_stop == 0 &&
    ...
    if (!monad_exec_iter_rewind_for_simple_replay(
            &ring_iter, missing.block_start, &live_event, &payload)) {
