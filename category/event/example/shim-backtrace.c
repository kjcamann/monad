...
// a "very nice" stack trace before printing the assertion death message, and
// then calling abort(3).
//
...
// the DWARF debugging information in the ELF files loaded by the dynamic
// linker. Without this, only symbols in dynamic libraries will be visible, and
// even then will not contain source code locations.
//
...
//      (available via <stacktrace>). It available in libstdc++, although not
...
//      libstdc++. The libstdc++ implementation uses the libbacktrace C
//      library provided by the gcc compiler project to do the heavy lifting
//
//   3. If you are using pure C, you can directly use libbacktrace yourself;
//      despite the name, this should _not_ be confused with the backtrace(3)
//      function in <execinfo.h>, which is provided by glibc; libbacktrace is a
//      large-ish library that can support more sophisticated symbolization
//
