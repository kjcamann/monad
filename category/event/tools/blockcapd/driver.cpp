...
std::sig_atomic_t g_scrape_signaled;
std::function<void(LogMessage const &)> g_log_writer;
...
Options:
  ...
  -r | --prometheus-relay <path> path to prometheus relay UNIX socket
...
struct option const longopts[] = {
    ...
    {"prometheus-relay", required_argument, nullptr, 'r'},
...
int parse_options(int argc, char **argv, BlockCapOptions *options)
{
    ...
    while ((ch = getopt_long(
                argc, argv, "he:t:r:f:s:z:i:", longopts, nullptr)) != -1) {
        switch (ch) {
        case 'h':
            help();
        ...
        case 'r':
            options->prom_relay_path = optarg;
            break;
...
void scrape_signal_handler(int)
{
    g_scrape_signaled = 1;
}
...
void default_log_writer(LogMessage const &m)
{
...
{
    g_log_writer = default_log_writer;
    ...
    if (!options.prom_relay_path.empty()) {
        std::signal(SIGUSR1, scrape_signal_handler);
    }
