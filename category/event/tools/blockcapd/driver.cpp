...
Positional arguments:
  <block-archive-dir>   path of execution event ring shared memory file)",
...
monad_bcap_block_archive *open_block_archive(char const *path)
{
    ...
    int const fd = open(path, O_DIRECTORY | O_PATH);
    if (fd == -1) {
        err(EX_OSERR, "open of block archive directory `%s` failed", path);
    }
    if (monad_bcap_block_archive_open(&block_archive, fd, path) != 0) {
        errx(
            EX_SOFTWARE,
...
int main(int argc, char **argv)
{
    ...
    monad_bcap_block_archive *const block_archive =
    ...
    capture_blocks(&options, block_archive);
