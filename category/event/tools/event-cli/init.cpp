    ...
    std::string_view resolve_context)
{
    ...
    int const rc = stat(input_file.c_str(), &input_file_stat);
    ...
    if (rc != 0) {
        ...
        errc_f(
            EX_CONFIG,
            std::errc{rc},
            "could not stat(2) event source file path `{}`",
            input_file.string());
    }

    // Canonicalize the file name so we can insert it into the named input list
    std::error_code canonicalize_err;
    fs::path origin_path = weakly_canonical(input_file, canonicalize_err);
    if (canonicalize_err == std::errc::no_such_file_or_directory &&
        input_file_stat.st_nlink == 0) {
        // The file itself exists (or stat(2) would have returned ENOENT and we
        // wouldn't get this far) but the canonicalization of the file returned
        // ENOENT. There are also no hard links to the file at all. This means
        // the user is trying to open an event source file with no hard links in
        // any filesystem.
        //
        // This could be a file whose unlink(2) we were racing against, but it
        // is more likely a proc(5) filesystem entry to a memfd_create(2)
        // descriptor, and we're looking at a temporary in-memory event ring,
        // e.g., from `monad_event_ring_create_ephemeral`. In either case, just
        // accept the original name as canonical.
        origin_path = input_file;
    }
    else if (canonicalize_err.value() != 0) {
        // Any other canonicalization errors are fatal.
        errc_f(EX_CONFIG, canonicalize_err.default_error_condition(),
            "could not canonicalize event source file path `{}`",
            input_file.string());
    }
