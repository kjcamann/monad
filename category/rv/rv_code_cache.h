...
void monad_rv_code_cache_insert_valid(
    struct monad_rv_code_cache *, struct monad_address const *,
    struct monad_bv db_code, struct monad_rv_code_zstd_decomp *,
    monad_rv_code_token_t *);
...
/// if the return value is MONAD_RV_VALIDATE_OK
monad_rv_validate_result_t monad_rv_code_cache_try_insert_new(
    struct monad_rv_code_cache *, struct monad_address const *,
    struct monad_bv txn_data, struct monad_rv_code_sections *, bool strict_rv64,
    struct monad_rv_code_zstd_decomp *, monad_rv_code_token_t *);

/// Given a code token, return the native execution handle that must be passed
/// to the `evmc_execute` function
void *monad_rv_code_token_native_handle(monad_rv_code_token_t);
