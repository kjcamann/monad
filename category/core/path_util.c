    ...
    int *final_dirfd, char *pathbuf, size_t pathbuf_size)
{
    ...
    if (*path_suffix == '/' && curfd == AT_FDCWD) {
        // Properly support absolute paths with AT_FDCWD; we wouldn't get it
        // right without this special case because we walk relative paths one
        // path component at a time, so e.g., "/tmp/xyz" would accidentally
        // mkdirat <cwd>/tmp on the first iteration
