...
#define MONAD_EVCAP_INLINE_ERR(...)                                            \
    _monad_evcap_set_inline_error(__FUNCTION__, __FILE__, __LINE__, __VA_ARGS__)
...
inline bool monad_evcap_event_iter_next(
    struct monad_evcap_event_iter *iter,
    struct monad_event_descriptor const **event, void const **payload)
{
...
inline bool monad_evcap_event_iter_prev(
    struct monad_evcap_event_iter *iter,
    struct monad_event_descriptor const **event, void const **payload)
{
    if (MONAD_UNLIKELY(
...
inline bool monad_evcap_event_iter_copy(
...
inline int monad_evcap_event_iter_set_seqno(
