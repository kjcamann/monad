...
static bool monad_evcap_event_iter_next(
...
static bool monad_evcap_event_iter_prev(
    struct monad_evcap_event_iter *, struct monad_event_descriptor const **,
    void const **payload);

/// Similar to monad_evcap_event_iter_next, but does not advance the iterator
static bool monad_evcap_event_iter_copy(
...
static int monad_evcap_event_iter_set_seqno(
