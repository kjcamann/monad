 ...
 *     while payload access "zero-copy"; payloads can also expire
 *
 * In the unified interface, descriptors are copied in both, and event payloads
 * pointers are always returned with descriptors. This table shows what the
 * implementation functions generally do:
 *
 ...
 * wrapper functions which do almost nothing. Many clever programming techniques
 ...
 * approaches using macros or templates would be able to reduce the source code
 ...
 * code, but all of the code is trivial "boilerplate" -- the kind that would
    ...
    struct monad_event_descriptor *event, void const **payload)
{
    ...
    if (monad_evcap_event_section_copy_seqno(
            event_section, seqno, &evcap_event, payload) == 0) {
        *event = *evcap_event;
    ...
    void const **payload)
{
    ...
    if (monad_evcap_event_iter_next(iter, &evcap_event, payload)) {
        *event = *evcap_event;
        return MONAD_EVSRC_SUCCESS;
    }
...
{
    struct monad_event_descriptor const *evcap_event;
    if (monad_evcap_event_iter_prev(iter, &evcap_event, payload)) {
        *event = *evcap_event;
        return MONAD_EVSRC_SUCCESS;
    }
    return MONAD_EVSRC_END;
...
inline int monad_evsrc_iter_set_seqno_ri(
...
inline int monad_evsrc_iter_set_seqno_ci(
    struct monad_evcap_event_iter *iter, uint64_t seqno)
{
    return monad_evcap_event_iter_set_seqno(iter, seqno) == 0
               ? MONAD_EVSRC_SUCCESS
               : MONAD_EVSRC_ERROR;
}
...
{
    struct monad_event_descriptor const *evcap_event;
    if (monad_evcap_event_iter_copy(iter, &evcap_event, payload)) {
        *event = *evcap_event;
        return MONAD_EVSRC_SUCCESS;
    }
    return MONAD_EVSRC_END;
