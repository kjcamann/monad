    ...
    size_t *map_len)
{
    ...
    *map_base = nullptr;
    snprintf(namebuf, sizeof namebuf, "sd:%lu", (long)sd->content_offset);
    ...
    *map_base =
    ...
    if (ZSTD_isError(zstd_rc)) {
        rc = FORMAT_ERRC(
            EIO,
            "ZSTD_decompress error for compressed %s section: %s",
            g_monad_evcap_section_names[sd->type],
    ...
    struct monad_evcap_reader *ecr, bool *invalidated)
{
    ...
    new_map_base = mmap(
        nullptr,
        (size_t)capture_stat.st_size,
        PROT_READ,
        MAP_SHARED,
        ecr->fd,
        0);
    if (new_map_base == MAP_FAILED) {
        return FORMAT_ERRC(errno, "mmap of refreshed mapping failed");
    }
#endif
...
// for now...
char const *g_monad_evcap_section_names[MONAD_EVCAP_SECTION_COUNT] = {
