...
#include <category/core/mem/cma/cma_fallback.h>

static int fallback_alloc(
    struct monad_cma_fallback *fb, size_t size, size_t align,
    struct monad_memblk *blk)
{
    ...
    rc = monad_cma_alloc(fb->primary, size, align, blk);
    if (MONAD_UNLIKELY(blk->ptr == nullptr)) {
        return monad_cma_alloc(fb->secondary, size, align, blk);
    }
    return rc;
}

static int cross_alloc_move(
    struct monad_allocator *from, struct monad_allocator *to, size_t size,
    size_t align, struct monad_memblk *old_blk)
{
    int rc;
    ...
    struct monad_memblk *blk)
{
    ...
    if (blk->ptr == nullptr || fb->primary->vtable->owns(fb->primary, *blk)) {
        return fb->primary->vtable->realloc(fb->primary, size, align, blk) ||
               cross_alloc_move(fb->primary, fb->secondary, size, align, blk);
    }
    return fb->secondary->vtable->realloc(fb->secondary, size, align, blk) ||
           cross_alloc_move(fb->secondary, fb->primary, size, align, blk);
}

static void
fallback_dealloc(struct monad_cma_fallback *fb, struct monad_memblk blk)
{
    if (fb->primary->vtable->owns(fb->primary, blk) == MONAD_CMA_OWNS_TRUE) {
        return monad_cma_dealloc(fb->primary, blk);
    }
    return monad_cma_dealloc(fb->secondary, blk);
}

static bool
fallback_owns(struct monad_cma_fallback *fb, struct monad_memblk blk)
{
    return fb->primary->vtable->owns(fb->primary, blk) ||
           fb->secondary->vtable->owns(fb->secondary, blk);
}

static struct monad_allocator_ops g_fallback_owns_ops = {
...
static struct monad_allocator_ops g_fallback_no_owns_ops = {
    .alloc = (monad_cma_alloc_fn *)fallback_alloc,
    .realloc = (monad_cma_realloc_fn *)fallback_realloc,
    .dealloc = (monad_cma_dealloc_fn *)fallback_dealloc,
    .owns = nullptr};
    ...
    struct monad_allocator *secondary, struct monad_allocator **ma)
{
    ...
    if (fallback->secondary->vtable->owns != nullptr) {
        fallback->self.vtable = &g_fallback_owns_ops;
    }
    else {
        fallback->self.vtable = &g_fallback_no_owns_ops;
    }
    if (ma != nullptr) {
        *ma = &fallback->self;
    }
