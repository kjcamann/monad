...
{
#if 0
    struct monad_rvc_elf const *image;
    Elf64_Ehdr const *elf_header;
    monad_rv_code_token_t code_token;

    code_token = *(monad_rv_code_token_t const *)raw_code;
    image = monad_rv_code_token_get_vm_handle(code_token);
    // XXX: elf_header = monad_rvc_elf_get_header(image);
    ...
    return host_exec_run(vm, host_if, host_ctx, rev, msg, code_token);
}
