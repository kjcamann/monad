    ...
    mrv_evm_exit_type_t const type, void const *const buf, size_t const len)
{
    ...
    register long t0 __asm__("t0") = RVI_ECALL_EVM_EXIT;
...
struct monad_bv mrv_evm_calldata()
{
    ...
    struct rv64_syscall_rpair const r =
        rv64_syscall0_r2(RVI_ECALL_EVM_CALLDATA);
...
struct monad_bv mrv_evm_returndata()
{
    struct monad_bv bv;
    struct rv64_syscall_rpair const r =
        rv64_syscall0_r2(RVI_ECALL_EVM_RETURNDATA);
    __builtin_memcpy(&bv, &r, sizeof r);
    return bv;
}
...
{
    return (uint64_t)rv64_syscall0(RVI_ECALL_EVM_GAS);
}

struct monad_bytes32 *
mrv_evm_keccak(void const *src, size_t n, struct monad_bytes32 *digest)
{
    (void)rv64_syscall3((long)src, (long)n, (long)digest, RVI_ECALL_EVM_KECCAK);
    return digest;
}
...
{
    (void)rv64_syscall2((long)key, (long)value, RVI_ECALL_EVM_SLOAD);
}

void mrv_evm_sstore(
    struct monad_bytes32 const *key, struct monad_bytes32 const *value)
{
    (void)rv64_syscall2((long)key, (long)value, RVI_ECALL_EVM_SSTORE);
}
