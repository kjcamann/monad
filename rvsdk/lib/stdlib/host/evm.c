    ...
    uint8_t *const codebuf, size_t const buflen)
{
    ...
    return ctx->host_if->copy_code(
        ctx->host_ctx, evmc_addr, offset, codebuf, buflen);
}
...
mrv_err_t mrv_evm_call(struct mrv_evm_call_args const *const call)
{
    ...
    ctx->last_call_result = ctx->host_if->call(ctx->host_ctx, &msg);
...
{
    mrv_host_exit((enum rvi_exit_type)type, buf, len);
}
