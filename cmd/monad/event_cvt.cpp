...
std::string as_hex_string(I i)
{
    return fmt::format("0x{:x}", i);
}
...
nlohmann::json to_alloy_json(BlockHeader const &eth_header)
{
    ...
    j["number"] = as_hex_string(eth_header.number);
    j["gasLimit"] = as_hex_string(eth_header.gas_limit);
    j["gasUsed"] = as_hex_string(eth_header.gas_used);
    j["timestamp"] = as_hex_string(eth_header.timestamp);
    ...
    // For some reason, block builder reassembly ties these to zero rather
    // than having them be null
    j["blobGasUsed"] = as_hex_string(eth_header.blob_gas_used.value_or(0));
    j["excessBlobGas"] = as_hex_string(eth_header.excess_blob_gas.value_or(0));
    j["parentBeaconBlockRoot"] =
        fmt::to_string(eth_header.parent_beacon_block_root.value_or({}));
    j["requestsHash"] = fmt::to_string(eth_header.requests_hash.value_or({}));
...
nlohmann::json to_alloy_json(Transaction const &txn)
{
    ...
    j["type"] = as_hex_string(std::to_underlying(txn.type));
    if (txn.sc.chain_id) {
        j["chainId"] = as_hex_string(static_cast<uint64_t>(*txn.sc.chain_id));
    }
    j["nonce"] = as_hex_string(txn.nonce);
    j["gasLimit"] = as_hex_string(txn.gas_limit);
    if (txn.to) {
    ...
    case TransactionType::eip2930:
        j["gasPrice"] =
            as_hex_string(static_cast<uint64_t>(txn.max_fee_per_gas));
        break;
    ...
    case TransactionType::eip7702:
        ...
        j["maxPriorityFeePerGas"] =
            as_hex_string(static_cast<uint64_t>(txn.max_priority_fee_per_gas));
        break;
    ...
    if (txn.type == TransactionType::legacy) {
        j["v"] = as_hex_string(static_cast<uint64_t>(get_v(txn.sc)));
    }
    else {
        ...
        j["yParity"] = as_hex_string(txn.sc.y_parity ? 1 : 0);
    }
    ...
    if (txn.type == TransactionType::eip7702) {
        ...
        for (AuthorizationEntry const &entry : txn.authorization_list) {
            ...
            entry_json["yParity"] = as_hex_string(entry.sc.y_parity ? 1 : 0);
...
void update_storage_slots(
    ...
    AccountState::StorageMap const *modified_storage,
    BlockAccountInfo::storage_map *merged_map)
{
    for (auto const &[key, value] : *prestate_storage) {
        ...
        if (modified_storage) {
            if (bytes32_t const *const new_value = modified_storage->find(key);
                new_value != nullptr &&
                *new_value != i_slot->second.current_value()) {
                i_slot->second.modified_value = *new_value;
            }
        }
    }
}
    ...
    std::unordered_map<Address, BlockAccountInfo> &account_map)
{
    ...
    for (auto const &[addr, original_acct_state] : state.original()) {
        ...
        if (inserted) {
            std::optional<Account> const &opt_account =
                get_account_for_trace(original_acct_state);
            // The first time we see it, copy it. Otherwise, we only
            // change it when it's modified
            account_info.original_nonce = opt_account ? opt_account->nonce : 0;
            account_info.original_balance =
                opt_account ? opt_account->balance : 0;
            account_info.code_hash =
                opt_account ? opt_account->code_hash : NULL_HASH;
        }
        ...
        if (modified_state) {
            ...
            if (is_dead(modified_account)) {
                modified_nonce = 0;
                modified_balance = 0;
            }
            else {
                modified_nonce = modified_account->nonce;
                modified_balance = modified_account->balance;
            }
    ...
    AccountState::StorageMap const *modified_storage)
{
    ...
    for (auto const &[key, value] : *prestate_storage) {
        ...
        if (modified_storage) {
            if (bytes32_t const *const new_value = modified_storage->find(key);
                new_value != nullptr && *new_value != value) {
                storage_slot_json["modified_value"] =
                    fmt::to_string(*new_value);
            }
        }
...
nlohmann::json state_to_json(State const &state)
{
    ...
    for (auto const &[addr, original_acct_state] : state.original()) {
        ...
        std::optional<Account> const &original_account =
            get_account_for_trace(original_acct_state);
        if (!is_dead(original_account)) {
...
struct ExpectedDataRecorder::Impl
{
    ...
    std::vector<nlohmann::json> txn_call_frames;
    std::vector<nlohmann::json> txn_states;
    nlohmann::json prologue_state_json;
    nlohmann::json epilogue_state_json;
};
...
void ExpectedDataRecorder::visit_transaction_state(
    unsigned txn_num, State const &s)
{
    update_block_account_info(s, impl_->block_account_map);
    MONAD_ASSERT_PRINTF(
        txn_num == impl_->txn_states.size(),
        "txn_num: %u != txn_states.size(): %lu",
        txn_num,
        impl_->txn_states.size());
    impl_->txn_states.emplace_back(state_to_json(s));
}

void ExpectedDataRecorder::visit_prologue_state(State const &s)
{
    update_block_account_info(s, impl_->block_account_map);
    impl_->prologue_state_json = state_to_json(s);
}

void ExpectedDataRecorder::visit_epilogue_state(State const &s)
{
    update_block_account_info(s, impl_->block_account_map);
    impl_->epilogue_state_json = state_to_json(s);
}
...
void ExpectedDataRecorder::record_execution(
    ...
    std::span<Transaction const> txns, std::span<Address const> senders,
    std::span<Receipt const> receipts)
{
    ...
    for (size_t i = 0; i < size(txns); ++i) {
        MONAD_ASSERT_PRINTF(
            i < impl_->txn_call_frames.size(),
            "i=%lu >= txn_call_frames.size()=%lu; missing --trace_calls?",
            i,
            impl_->txn_call_frames.size());
        ...
        txn_json["call_frames"] = std::move(impl_->txn_call_frames[i]);
        txn_json["account_accesses"] = std::move(impl_->txn_states[i]);
    ...
    j["prologue_account_accesses"] = std::move(impl_->prologue_state_json);
    j["epilogue_account_accesses"] = std::move(impl_->epilogue_state_json);
    j["all_account_accesses"] = to_json(impl_->block_account_map);
    ...
    impl_->block_account_map.clear();
    impl_->txn_call_frames.clear();
    impl_->txn_states.clear();
}
...
CVTCallTracer::CVTCallTracer(
    Transaction const &txn, std::vector<CallFrame> &call_frames,
    unsigned txn_num, ExpectedDataRecorder *cvt_recorder)
    : CallTracer{txn, call_frames}
    , txn_num_{txn_num}
    , cvt_recorder_{cvt_recorder}
{
}
...
void CVTCallTracer::on_finish(uint64_t const gas_used)
{
    ...
    if (cvt_recorder_ != nullptr) {
        for (CallFrame const &call_frame : this->get_call_frames()) {
            call_frames_json_array.emplace_back(to_alloy_json(call_frame));
        }
        MONAD_ASSERT(txn_num_ == cvt_recorder_->impl_->txn_call_frames.size());
        cvt_recorder_->impl_->txn_call_frames.emplace_back(
            std::move(call_frames_json_array));
    }
}
