...
template <typename... Args>
void warnc_f(
    std::error_condition const &ec, std::format_string<Args...> fmt,
    Args &&...args)
{
    std::print(stderr, "{}: ", __progname);
    std::print(stderr, fmt, std::forward<Args>(args)...);
...
template <typename... Args>
void warnx_f(std::format_string<Args...> fmt, Args &&...args)
{
    warnc_f({}, fmt, std::forward<Args>(args)...);
}

template <typename T>
T unwrap_or_err(std::expected<T, std::string> u)
{
    if (!u) {
        errx_f(1, "{}", u.error());
    }
