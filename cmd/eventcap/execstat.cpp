    ...
    void const *payload, State &state)
{
    ...
    case MONAD_EXEC_BLOCK_START: {
        ...
        if (!iter->check_payload(event)) {
            return on_payload_expired(current_block, event, iter, stderr);
        }
    }
        return false;
...
void execstat_thread_main(std::span<Command *const> commands)
{
        ...
        case Success:
            ...
            if (process_event(&iter, &event, payload, state)) {
                ...
                if (tty_control_codes) {
                    if (num_blocks_processed == 1) {
                        std::print(output, ANSI_ClearScreen);
                    }
                ...
                print_block_update(state, output);
                act_on_block_end(state);

                if (tty_control_codes) {
                    std::print(output, "{}", ANSI_FinishUpdate);
                }
