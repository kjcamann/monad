...
MiB/s payload consumption rate.)");
    ...
    record
        ->add_option(
            "-z,--event-zstd-level",
            record_command.event_zstd_level,
...
The threading model behaves the same as the dump command.)");
    ...
    for (std::unique_ptr<Command> const &command : topology.commands) {
        ...
        if (command->has_type(Command::Type::SectionDump)) {
            auto const *const options =
                command->get_options<SectionDumpCommandOptions>();
            // The loop in dump_evcap_sections is "merge-like"; it requires the
            // section index inputs to be sorted
            std::vector<unsigned> sorted_sections = options->sections;
            std::ranges::sort(sorted_sections);
            dump_evcap_sections(
                options,
                static_cast<EventCaptureFile *>(
                    command->event_sources[0].source_file),
                sorted_sections,
                command->output->file);
        }
    }
