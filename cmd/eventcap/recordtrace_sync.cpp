...
void recordtrace_sync(
    std::stop_token stop, RecordTraceSettings const *config,
    RecordTraceState *rts)
{
    ...
    ZSTD_CCtx *const event_zstd_cctx =
        unwrap_or_err(create_zstd_cctx(config->options->event_zstd_level));
    ZSTD_CCtx *const seqno_index_zstd_cctx =
        unwrap_or_err(create_zstd_cctx(config->options->seqno_zstd_level));

    BC_CHECK(monad_bcap_block_archive_open(
        &block_archive, config->output_fd, config->output_spec.c_str()));

    rts->start_latch->arrive_and_wait();
    while (g_should_exit == 0 && !stop.stop_requested()) {
