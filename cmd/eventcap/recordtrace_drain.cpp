...
void drain_trace_events(
    std::stop_token &stop, TraceEventSourceState *state,
    BlockTrace *cur_block_trace)
{
    ...
    while (g_should_exit == 0 && !stop.stop_requested() && !state->finished) {
...
void recordtrace_drain(
    ...
    monad_vbuf_segment_allocator *segment_allocator,
    RecordTraceSettings const *, RecordTraceState *rts)
{
