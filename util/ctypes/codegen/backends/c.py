...
def emit_module_header_prologue(emit_warning: bool, module: ModuleInfo, out: TextIO):
  ...
  if module.is_event_module:
    module_include_lines.append('#include <monad/event/event_metadata.h>')
...
def emit_enum_type(
  ...
  print(f'enum {enum_type_name} : {storage_type_name} {{', file=out)
...
def emit_struct_type(
  ...
  print(f"""struct {struct_type_name} {{""", file=out)
...
def emit_module_header_file(
  ...
  for type_info in module.type_defs.values():
    if isinstance(type_info, EnumTypeInfo):
      ...
      pass # Externally defined; we don't emit these
    else:
...
extern uint8_t const g_{event_type_prefix}_event_metadata_hash[32];

#define MONAD_EVENT_DEFAULT_{event_ring_type.upper()}_RING_PATH \\
...
f'''struct monad_event_metadata const g_{event_type_prefix}_event_metadata[{n_types}] = {{
  ...
  print(f'uint8_t const g_{event_type_prefix}_event_metadata_hash[32] = {{', file=out)
  ...
  for module_name, module in module_map.items():
    ...
    if module.is_event_module:
      ...
      if args.stdout:
        ...
        with open(args.path / metadata_file_name, 'wt') as out:
          emit_event_metadata_file(args, module, out)
        try_run_clang_format(args.clang_format, clang_format_args,
                             args.path / metadata_file_name)
